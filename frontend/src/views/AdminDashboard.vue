  ...
  <div class="admin-root">
    ...
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      <div class="sidebar-header">
        <img src="/logo.png" class="sidebar-logo" alt="" @error="e=>e.target.style.display='none'" />
    ...
    <div class="main">
      ...
      <header class="topbar">
        ...
        <div class="topbar-status">
          <span class="status-dot" :class="{ live: status.isLive }"></span>
          <span class="status-txt">{{ status.isLive ? 'LIVE' : (status.isPlaying ? 'Playing' : 'Idle') }}</span>
          <span class="listener-badge">{{ status.listeners || 0 }} 👥</span>
        </div>
      </header>
...
const nowPlayingLabel = computed(() => {
  if (status.value.isLive) return '🔴 LIVE'
...
async function refresh() {
  try {
    settings.value = await adminStore.loadSettings().then(() => adminStore.settings)
    status.value   = await adminStore.getLiveStatus()
  } catch {}
}
...
.footer-link {
  font-size: 12px; color: #c0a8d8; text-decoration: none; padding: 7px 10px;
  border-radius: 7px; transition: background 0.15s;
}
.footer-link:hover { background: rgba(212,168,67,0.08); color: #d4a843; }
.logout-btn {
  padding: 8px 10px; border-radius: 7px; border: 1px solid rgba(224,48,96,0.25);
  background: rgba(224,48,96,0.06); color: #ff80a0; font-size: 12px; cursor: pointer;
...
.menu-btn {
  display: none;
  border: none; background: transparent; color: #d4a843; font-size: 20px; cursor: pointer;
}
...
.pctrl-btn {
  width: 32px; height: 32px; border-radius: 50%; border: 1px solid; cursor: pointer;
